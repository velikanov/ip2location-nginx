...
ngx_http_ip2location_country_code(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data){
	...
	char ip_address_text[INET_ADDRSTRLEN];
	inet_ntop(AF_INET, &(sin->sin_addr), ip_address_text, INET_ADDRSTRLEN);

	IP2LocationRecord *record = IP2Location_get_country_short((IP2Location *)data, ip_address_text);
...
ngx_http_ip2location_country_name(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data){
	...
	IP2LocationRecord *record = IP2Location_get_country_long((IP2Location *)data, ip_address_text);
